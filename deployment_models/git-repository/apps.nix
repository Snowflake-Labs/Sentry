/**
  Contains Nix flake application to generate the documentation for stored procedure distribution through Snowflake git integration.
...
in
{
  mkSprocDocs = wrapPythonScript {
    name = "mkSprocDocs";
    pythonScript = "from scripts import render_stored_procedures; render_stored_procedures()";
  };
  mkSingleCreateSprocFile = wrapPythonScript {
    name = "mkSingleCreateSprocFile";
    pythonScript = "from scripts import render_sprocs_as_single_file; render_sprocs_as_single_file()";
  };
}
